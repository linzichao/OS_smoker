...
public class SmokersAgents{
	...
	public void setAgents(Agent[] agts) {
		this.agts = agts;
	}
	...
	//	private static CyclicBarrier barrier = new CyclicBarrier(3);
		...
		public void run() {
			...
			while(true) {    
				System.out.print("DB_1");
				putIngred();
				System.out.print("DB_2");
				/*
				try {
					latches[latchCount % 2].await();
				}
				catch (InterruptedException e) {}
				*/
			}
	...
	class Smoker extends Thread{
		...
		synchronized void tryCountDown() {
			for(Agent agt : agts) {
				if(agt.getState() != Thread.State.WAITING) {
					break;
				}
			}
			latchCount ++;
			latches[latchCount % 2] = new CountDownLatch(1);
			latches[(latchCount-1) % 2].countDown();
		}
